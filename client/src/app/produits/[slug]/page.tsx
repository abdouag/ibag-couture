...
export default async function ProductPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-brand-cream">
      ...
      />
      {/* Breadcrumb - LUXURY */}
      <div className="pt-6 md:pt-8 lg:pt-10 pb-4 md:pb-6 bg-white relative grain-texture border-b border-stone-200">
        <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          <nav className="flex items-center gap-2 text-sm md:text-base text-stone-400">
            ...
            <span className="text-noir font-medium truncate">{product.name}</span>
          </nav>
        </div>
      </div>
            ...
            <div className="lg:py-4">
              <div className="sticky top-24 md:top-28 lg:top-32 space-y-6 md:space-y-8">
                ...
                <div>
                  ...
                  <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-luxury text-noir leading-tight mb-3">
                    {product.name}
                  </h1>
                ...
                <div className="relative bg-white rounded-sm p-6 md:p-8 border border-stone-200 shadow-lg">
                  <div className="absolute inset-0 shimmer-gold rounded-sm" />
                  {product.promoPrice != null && product.promoPrice < product.basePrice ? (
                    ...
                    <>
                      ...
                      <p className="text-4xl md:text-5xl lg:text-6xl font-luxury text-noir tracking-tight">
                  ...
                  {product.options && product.options.length > 0 && (
                    <div className="mt-6 pt-6 border-t border-stone-200">
                      <div className="flex items-baseline justify-between">
                        ...
                        <span className="text-2xl md:text-3xl font-luxury text-brand-gold font-medium">
                      ...
                      {product.options.map((option, index) => (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className="w-6 h-6 bg-brand-gold rounded-full flex items-center justify-center flex-shrink-0">
                ...
                {product.availableSizes && product.availableSizes.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <h2 className="text-xs md:text-sm tracking-[0.25em] uppercase text-stone-400 font-light">
                        Tailles disponibles
                      </h2>
                      <SizeGuideModal
                    ...
                    <div className="flex flex-wrap gap-3">
                      {product.availableSizes.map((size) => (
                        <span
                          key={size}
                          className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center border-2 border-stone-200 hover:border-brand-gold text-stone-700 text-base md:text-lg font-medium rounded-sm hover:shadow-md transition-all duration-300 cursor-pointer"
                        >
                          {size}
                        </span>
                      ))}
                      {product.isCustomAvailable && (
                        <span className="h-14 md:h-16 px-5 md:px-6 flex items-center justify-center border-2 border-brand-gold/30 bg-brand-cream text-brand-gold-dark text-sm md:text-base font-medium rounded-sm hover:bg-brand-cream transition-all duration-300">
                          Sur-mesure
                        </span>
                      )}
                    </div>
                ...
                {(!product.availableSizes || product.availableSizes.length === 0) && product.isCustomAvailable && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-4 bg-brand-cream p-5 md:p-6 rounded-sm border border-brand-gold/30 shadow-sm">
                      <svg className="w-6 h-6 text-brand-gold flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                <div className="space-y-4 md:space-y-5">
                  {isOutOfStock ? (
                    ...
                    <Link
                      ...
                      className="group relative block w-full bg-noir text-white text-center py-5 md:py-6 lg:py-7 text-base md:text-lg tracking-[0.2em] uppercase font-medium overflow-hidden transition-all duration-500 hover:bg-stone-800 hover:shadow-2xl rounded-sm hover:scale-[1.02] btn-luxury"
                    >
                ...
                <div className="grid grid-cols-3 gap-4 pt-6 border-t border-stone-200">
                  <div className="text-center p-3 group">
                    <div className="w-12 h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 mx-auto mb-3 bg-brand-cream rounded-full flex items-center justify-center group-hover:bg-stone-200 transition-all duration-300">
                      <svg className="w-6 h-6 md:w-7 md:h-7 lg:w-8 lg:h-8 text-brand-gold-dark" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
                ...
                <div className="flex items-center gap-3 text-stone-400 text-sm md:text-base">
                  ...
                  <span>Des questions ? <Link href="/contact" className="text-brand-gold hover:underline transition-colors">Contactez-nous</Link> ou <a href="https://wa.me/221770470928" target="_blank" rel="noopener noreferrer" className="text-[#25D366] hover:underline transition-colors">WhatsApp</a></span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6 sm:px-8 lg:px-12">
          <div className="bg-white rounded-sm p-8 md:p-12 lg:p-16 shadow-xl">
            <div className="text-center mb-10 md:mb-12">
              <h3 className="font-luxury text-3xl md:text-4xl lg:text-5xl text-noir mb-4">Guide des mesures</h3>
              <p className="text-stone-500 text-base md:text-lg mb-8">Chaque pièce est réalisée sur mesure. Vous aurez besoin de :</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6 mb-10">
              {[
                ...
                <div key={measure.label} className="flex items-start gap-3 p-5 bg-stone-50 rounded-sm border border-stone-200 hover:border-brand-gold/30 hover:bg-brand-cream transition-all duration-300">
                  <svg className="w-5 h-5 text-brand-gold flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                  <div>
                    <span className="text-sm md:text-base text-stone-700 font-medium block">{measure.label}</span>
                    <span className="text-xs text-stone-400">{measure.desc}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="border border-stone-200 rounded-sm overflow-hidden">
              <div className="bg-stone-50 px-6 py-4 border-b border-stone-200">
                <h4 className="text-sm tracking-[0.2em] uppercase text-stone-600 font-medium">Correspondance des tailles (cm)</h4>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm md:text-base">
                  <thead>
                    <tr className="border-b border-stone-200 bg-stone-50/50">
                      <th className="py-3 px-4 text-left text-xs tracking-[0.15em] uppercase text-stone-400 font-medium">Taille</th>
                      <th className="py-3 px-4 text-center text-xs tracking-[0.15em] uppercase text-stone-400 font-medium">Poitrine</th>
                      <th className="py-3 px-4 text-center text-xs tracking-[0.15em] uppercase text-stone-400 font-medium">Taille</th>
                      <th className="py-3 px-4 text-center text-xs tracking-[0.15em] uppercase text-stone-400 font-medium">Hanches</th>
                      <th className="py-3 px-4 text-center text-xs tracking-[0.15em] uppercase text-stone-400 font-medium">Épaules</th>
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={row.size} className="border-b border-stone-100 hover:bg-brand-cream/50 transition-colors">
                        <td className="py-3 px-4">
                          <span className="inline-flex items-center justify-center w-9 h-9 bg-noir text-white text-sm font-medium rounded-sm">{row.size}</span>
                        </td>
                        <td className="py-3 px-4 text-center text-stone-700">{row.chest}</td>
                        <td className="py-3 px-4 text-center text-stone-700">{row.waist}</td>
                        <td className="py-3 px-4 text-center text-stone-700">{row.hips}</td>
                        <td className="py-3 px-4 text-center text-stone-700">{row.shoulders}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 sm:px-8 lg:px-12 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-5 justify-center">
            <a
              ...
              className="inline-flex items-center justify-center gap-3 border-2 border-[#25D366] text-[#25D366] px-10 md:px-12 py-4 md:py-5 text-sm md:text-base tracking-[0.2em] uppercase font-medium hover:bg-[#25D366] hover:text-white transition-all duration-500 hover:scale-105 btn-luxury"
            >
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:hidden bg-white/95 backdrop-blur-lg border-t-2 border-brand-gold/20 shadow-[0_-8px_16px_-4px_rgba(0,0,0,0.1)] z-50 px-5 py-3.5">
        <div className="flex items-center gap-4">
          <div className="flex-1 min-w-0">
            {product.promoPrice != null && product.promoPrice < product.basePrice ? (
              ...
              <p className="text-lg font-luxury text-noir font-medium">
          ...
          {isOutOfStock ? (
            ...
            <div className="flex items-center gap-2.5">
              ...
              <Link
                href={`/commander/${product.slug || product._id}`}
                className="bg-noir text-white px-6 py-3 text-xs tracking-[0.15em] uppercase font-medium hover:bg-stone-800 transition-all duration-300 whitespace-nowrap rounded-sm shadow-lg btn-luxury"
              >
                Commander
              </Link>
            </div>
