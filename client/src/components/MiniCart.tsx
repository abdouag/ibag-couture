    ...
    <div className="fixed inset-0 z-[60]">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl flex flex-col animate-slideInRight">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E8E4E0]">
          <h2 className="text-lg font-serif text-[#0D0D0D]">
            Votre Panier
            {items.length > 0 && (
          ...
          <button
            onClick={() => setIsCartOpen(false)}
            className="p-2 text-stone-400 hover:text-[#0D0D0D] transition-colors"
          >
        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
            <div className="w-20 h-20 border border-[#E8E4E0] rounded-full flex items-center justify-center mb-6">
            ...
            <p className="text-[#0D0D0D] font-medium mb-2">Votre panier est vide</p>
            <p className="text-stone-500 text-sm mb-6">Decouvrez nos creations et ajoutez vos coups de coeur.</p>
            <Link
              href="/collections"
              onClick={() => setIsCartOpen(false)}
              className="inline-block border border-[#0D0D0D] text-[#0D0D0D] px-8 py-3 text-sm tracking-wide hover:bg-[#0D0D0D] hover:text-white transition-all duration-300"
            >
          ...
          <>
              ...
              {items.map((item) => (
                <div key={item.productId} className="flex gap-4 px-6 py-4 border-b border-stone-100">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-[#C9A45C] uppercase tracking-wide">{item.category}</p>
                    <Link
                      href={`/produits/${item.slug}`}
                      onClick={() => setIsCartOpen(false)}
                      className="text-sm font-medium text-[#0D0D0D] hover:text-[#C9A45C] transition-colors truncate block"
                    >
                      {item.name}
                    </Link>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <div className="flex items-center border border-[#E8E4E0] rounded">
                        <button
                          ...
                          className="px-2 py-1 text-stone-500 hover:text-[#0D0D0D] transition-colors text-sm"
                        >
                          -
                        </button>
                        <span className="px-2 py-1 text-sm text-[#0D0D0D] min-w-[24px] text-center">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() => updateQuantity(item.productId, item.quantity + 1)}
                    ...
                    <Link
                      ...
                      onClick={() => setIsCartOpen(false)}
                      className="inline-block mt-2 text-xs text-[#C9A45C] hover:text-[#A67C3D] font-medium transition-colors"
                    >
            ...
            <div className="border-t border-[#E8E4E0] px-6 py-5">
              ...
              <a
                ...
                className="w-full flex items-center justify-center gap-2 bg-[#0D0D0D] text-white py-3.5 text-sm tracking-wide font-medium hover:bg-stone-800 transition-colors"
              >
              ...
              <button
                onClick={() => setIsCartOpen(false)}
                className="w-full border border-[#E8E4E0] text-stone-700 py-3 text-sm tracking-wide hover:bg-[#E8E4E0] transition-colors mt-2"
              >
